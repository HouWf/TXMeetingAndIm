...
- (void)setGroupMemberInfo:(NSString*)groupID userId:(NSString *)userId nameCard:(NSString *)nameCard callback:(TRTCMeetingCallback)callback{
    __weak __typeof(self) weakSelf = self;
    [[TXRoomService sharedInstance] setGroupMemberInfo:groupID userId:userId nameCard:nameCard callback:^(NSInteger code, NSString * _Nullable message) {
        __strong __typeof(weakSelf) self = weakSelf;
        if (self && callback) {
            [self asyncRunOnDelegateQueue:^{
                callback(code, message);
...
- (void)enterMeeting:(UInt32)roomId callback:(TRTCMeetingCallback)callback {
    [self asyncRun:^(TRTCMeeting *self) {
        ...
        [[TXRoomService sharedInstance] enterRoom:[NSString stringWithFormat:@"%u", roomId] callback:^(NSInteger code, NSString *message) {
            // 先不管这个
            // 增加不存在逻辑
            if(code != 0 || [message hasSuffix:@"not exist"] || [message containsString:@"not exist"]){
                callback(code, message);
                return;
            }
        }];
...
//        }];
        ...
        [[TXRoomService sharedInstance] cleanStatus];
    }];
}
...
- (void)stopLocalEvent {
    ...
    [[TRTCCloud sharedInstance] stopLocalPreview];
//    [[TRTCCloud sharedInstance] exitRoom];
    [[TXRoomService sharedInstance] exitRoom:^(NSInteger code, NSString * _Nullable message) {
...
- (void)getGroupMemberListGroupId:(NSString *)groupId callback:(TRTCMeetingUserListCallback)callback{
    [self asyncRun:^(TRTCMeeting *self) {
        NSMutableArray *userIdArray = [[NSMutableArray alloc] init];
        for (NSString *userId in self->_userInfoList) {
            [userIdArray addObject:userId];
        }
        [self getGroupMembersInfo:groupId memberList:userIdArray callback:callback];
    }];
}

- (void)getGroupMembersInfo:(NSString*)groupID memberId:(NSString *)memberId callback:(TRTCMeetingUserListCallback)callback;
{
    [self asyncRun:^(TRTCMeeting *self) {
        // 判断是否是辅流
        NSString *realUserId = [self->_substreamMap valueForKey:memberId];
        if (realUserId) {
            // 辅流直接从现有的取数据
            TRTCMeetingUserInfo *userInfo = [[TRTCMeetingUserInfo alloc] init];
            userInfo.userId = memberId;
            
            TRTCMeetingUserInfo *mainUserInfo = [self->_userInfoList valueForKey:memberId];
            if (mainUserInfo) {
                userInfo.userName = [NSString stringWithFormat:@"%@（%@）", mainUserInfo.userName, MeetingLocalize(@"Demo.TRTC.Meeting.secondarystream")];
                userInfo.avatarURL = mainUserInfo.avatarURL;
                userInfo.nameCard = mainUserInfo.nameCard;
            }
        ...
        // 不是辅流就走这里
        NSArray *userIdArray = [[NSArray alloc] initWithObjects:memberId, nil];
        [self getGroupMembersInfo:groupID memberList:userIdArray callback:callback];
    }];
}

- (void)getGroupMembersInfo:(NSString*)groupID memberList:(NSArray<NSString*>*)memberList callback:(TRTCMeetingUserListCallback)callback{
    ...
    [[TXRoomService sharedInstance] getGroupMembersInfo:groupID memberList:memberList callback:^(NSInteger code, NSString * _Nullable message, NSArray<TXUserInfo *> * _Nullable userList) {
        __strong __typeof(weakSelf) self = weakSelf;
        if (self == nil) {
            return;
        }
        ...
        [self asyncRun:^(TRTCMeeting *self) {
            ...
            for (TXUserInfo *info in userList) {
                ...
                if (userInfo) {
                    ...
                    userInfo.nameCard = info.nameCard;
...
- (void)updateRemoteView:(TXView *)view streamType:(TRTCVideoStreamType)streamType forUser:(NSString *)userId{
    [[TRTCCloud sharedInstance] updateRemoteView:view streamType:streamType forUser:userId];
}
...
-(void)setSystemVolumeType:(TXSystemVolumeType)type{
    [[TRTCCloud sharedInstance].getDeviceManager setSystemVolumeType:type];
}
...
//    [[TRTCCloud sharedInstance] startScreenCaptureByReplaykit:params appGroup:@"group.com.luneng.mobileApp"];
    [[TRTCCloud sharedInstance] startScreenCaptureByReplaykit:TRTCVideoStreamTypeSub encParam:params appGroup:@"NARI.mobileApp"];
}
...
- (void)sendC2CCustomMsg:(NSString *)cmd message:(NSString *)message to:(NSString *)userId callback:(TRTCMeetingCallback)callback {
    __weak __typeof(self) weakSelf = self;
    [[TXRoomService sharedInstance] sendC2CCustomMsg:cmd message:message to:userId callback:^(NSInteger code, NSString *message) {
        __strong __typeof(weakSelf) self = weakSelf;
        if (self && callback) {
            [self asyncRunOnDelegateQueue:^{
                callback(code, message);
...
- (void)kickedUser:(NSString *)userId callback:(TRTCMeetingCallback)callback{
    __weak __typeof(self) weakSelf = self;
    [[TXRoomService sharedInstance] kickedUser:userId callback:^(NSInteger code, NSString * _Nullable message) {
        __strong __typeof(weakSelf) self = weakSelf;
        if (self && callback) {
            [self asyncRunOnDelegateQueue:^{
                callback(code, message);
...
// 切换用户角色
- (void)transferGroupOwner:(NSString *)userId callback:(TRTCMeetingCallback) callback{
    __weak __typeof(self) weakSelf = self;
    [[TXRoomService sharedInstance] transferGroupOwner:userId callback:^(NSInteger code, NSString * _Nullable message) {
        __strong __typeof(weakSelf) self = weakSelf;
        if (self && callback) {
            [self asyncRunOnDelegateQueue:^{
                callback(code, message);
...
- (void)getGroupHistoryMessageListWithCount:(int)count lastMsg:(V2TIMMessage* __nullable)lastMsg succ:(V2TIMMessageListSucc)succ fail:(V2TIMFail)fail{
    NSString *roomId = [NSString stringWithFormat:@"%u", self.roomId];
    [[TXRoomService sharedInstance] getGroupHistoryMessageList:roomId count:count lastMsg:lastMsg succ:succ fail:fail];
}
...
- (void)onRemoteUserLeaveRoom:(NSString *)userId reason:(NSInteger)reason {
    [self asyncRun:^(TRTCMeeting *self) {
        ...
        if (self->_delegate && [self->_delegate respondsToSelector:@selector(onUserLeaveRoom:reson:)]) {
            [self asyncRunOnDelegateQueue:^{
                [self->_delegate onUserLeaveRoom:userId reson:reason];
            }];
        }
    }];
}
       ...
       // TODO 简单起见，伪造一个userId
        ...
        if (available) {
            ...
            [self asyncRunOnDelegateQueue:^{
                ...
                if (self->_delegate && [self->_delegate respondsToSelector:@selector(onUserLeaveRoom:reson:)]) {
                    [self->_delegate onUserLeaveRoom:subStreamUserId reson:0];
                }
            }];
        }
    }];
...
- (void)onC2CRecvRoomCustomMsg:(NSString *_Nullable)roomId cmd:(NSString *_Nullable)cmd message:(NSString *_Nullable)message userInfo:(TXUserInfo *_Nonnull)userInfo{
    [self asyncRunOnDelegateQueue:^{
        TRTCMeetingUserInfo *info = [[TRTCMeetingUserInfo alloc] init];
        info.userId = userInfo.userId;
        info.userName = userInfo.userName;
        ...
        if (self->_delegate && [self->_delegate respondsToSelector:@selector(onRecvC2CCustomMsg:message:userInfo:)]) {
            [self->_delegate onRecvC2CCustomMsg:cmd message:message userInfo:info];
        }
    }];
}
...
- (void)onMemberKickedWithOpUser:(TXUserInfo *_Nonnull)opUser memberList:(NSArray<TXUserInfo *>*_Nonnull)memberList{
    TRTCMeetingUserInfo *info = [[TRTCMeetingUserInfo alloc] init];
    info.userId = opUser.userId;
    info.userName = opUser.userName;
    info.avatarURL = opUser.avatarURL;
    
    NSMutableArray *memberListArr = NSMutableArray.array;
    for (NSInteger index = 0; index < memberList.count; index++) {
        TXUserInfo *model = memberList[index];
        TRTCMeetingUserInfo *memberInfo = [[TRTCMeetingUserInfo alloc] init];
        memberInfo.userId = model.userId;
        memberInfo.userName = model.userName;
        memberInfo.avatarURL = model.avatarURL;
        [memberListArr addObject:memberInfo];
    }
    
    if ([self.delegate respondsToSelector:@selector(onMemberKickedWithOpUser:memberList:)]) {
        [self.delegate onMemberKickedWithOpUser:info memberList:memberListArr];
    }
